...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/commcon-2024/browser/index.html': ['build/commcon-2024/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/commcon-2024/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/commcon-2024/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, extension) =>
                            grunt.file.expand({ cwd: 'build/commcon-2024', ext: extension }, `assets/${filename}.*`)[0]
                    },
                    {
                        match: /\s*"\/commcon-2024(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js",/g,
                        replacement: ''
                    },
                    {
                        match: /\s*"\/commcon-2024(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js":\s*"[\da-z]+",/g,
                        replacement: ''
                    },
                    {
                        // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                        match: /"\/commcon-2024\/(?<filename>index|start)\.html":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/commcon-2024/browser/${filename}.html": "${computeHashOfFile(
                                `build/commcon-2024/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/commcon-2024/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/commcon-2024/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(`build/commcon-2024/browser/${filename}`)}</script>`;
                        }
                    }
                ]
            }
